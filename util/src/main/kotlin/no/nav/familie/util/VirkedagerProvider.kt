...
class VirkedagerProvider {
    ...
    companion object {
        ...
        private fun beregnBevegeligeHelligdager(år: Int): Set<MonthDay> {
            ...
            return setOf(MonthDay.from(påskedag), MonthDay.from(skjærTorsdag), MonthDay.from(langfredag), MonthDay.from(andrePåskedag), MonthDay.from(kristiHimmelfartsdag), MonthDay.from(førstePinsedag), MonthDay.from(andrePinsedag))
        }
